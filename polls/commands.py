...
from polls.manager import (
    create_poll,
    generate_poll_id
)
from polls.ui import (
    build_poll_embed,
    PollView
)
...
from polls.non_voters import (load_non_voters, reset_user_absences, reset_all_absences)

from polls.polls_db import get_poll, get_votes
...
def setup_poll_commands(tree: app_commands.CommandTree, GUILD_ID: int):
    ...
    async def poll_simple(
        ...
        from polls.storage import save_poll
        save_poll(poll)

        from polls.polls_db import create_poll as db_create_poll
        db_create_poll(poll)
    ...
    async def poll_close(
        ...
        from polls.scheduler import close_poll_and_process
